...
def visualizar_serie(serie):
    ...
    serie.plot(ax=ax, color='navy', linewidth=1.2)
...
def analizar_residuo(residuo):
    ...
    fig.tight_layout()
    fig.savefig("output/ej4_acf_pacf.png", dpi=150)
    plt.close()
    
    # TODO: Histograma del residuo con curva normal superpuesta
    plt.figure(figsize=(8, 5))
    count, bins, ignored = plt.hist(residuo_limpio, bins=50, density=True, color='skyblue', edgecolor='black', alpha=0.7)
    x = np.linspace(residuo_limpio.min(), residuo_limpio.max(), 100)
    plt.plot(x, norm.pdf(x, media, std), color='red', linewidth=2, label=f'Normal teórica\nμ={media:.2f}, σ={std:.2f}')
    plt.title('Histograma del Residuo con Curva Normal Perfecta')
    ...
    with open("output/ej4_analisis.txt", "w", encoding='utf-8') as f:
        f.write("Análisis Estadístico del Componente de Residuo\n")
        f.write("="*50 + "\n")
        f.write(f"Media:     {media:.4f}\n")
        f.write(f"Std:       {std:.4f}\n")
        f.write(f"Asimetría: {asimetria:.4f}\n")
        f.write(f"Curtosis:  {curtosis:.4f}\n")
        f.write(f"Jarque-Bera p-value: {p_jb:.4e} ")
        f.write("(Ruido Gaussiano)\n" if p_jb > 0.05 else "(NO es Ruido Gaussiano estrictamente)\n")
        f.write(f"ADF Test p-value:    {p_adf:.4e} ")
        f.write("(Estacionario)\n" if p_adf < 0.05 else "(NO Estacionario)\n")
...
if __name__ == "__main__":
    ...
    for s in salidas:
        ...
        estado = "Path encontrado" if existe else "No encontrado"
